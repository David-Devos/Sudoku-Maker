...
private:
	...
	void SolveSudokuNaive(std::vector<int>& sudoku);
	bool FillNextNumberSolverNaive(int index, std::vector<int>& sudoku);

	bool SolveSudokuBacktracking(std::vector<int>& sudoku);
	bool FindUnassignedLocation(std::vector<int>& sudoku, int& index);
};